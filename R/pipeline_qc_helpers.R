...
build_assembly_gff_inventory <- function(metadata_file = FILE_METADATA,
  ...
  metadata_stale_messages <- c(
    ...
    if (any(metadata_linked$metadata_or_scan_status == "metadata_path_missing_on_disk")) {
      sprintf("%d metadata-linked FASTA path(s) are missing on disk.", sum(metadata_linked$metadata_or_scan_status == "metadata_path_missing_on_disk"))
    }
  )
  qc_stale_messages <- c(
...
append_denominator_summary <- function(df,
  ...
  status_col <- if ("Infection_Status" %in% names(df)) "Infection_Status" else if ("Status_Simple" %in% names(df)) "Status_Simple" else NA_character_
  ...
  row <- tibble::tibble(
    ...
    n_ASB = sum(statuses == "ASB", na.rm = TRUE),
    n_UTI = sum(statuses == "UTI", na.rm = TRUE),
    n_Negative = sum(statuses == "Negative", na.rm = TRUE),
...
write_uti_attrition_outputs <- function(status_file = file.path(DIR_CLINICAL, "status_map.csv"),
  ...
  uti <- status |> dplyr::filter(.data$Infection_Status == "UTI")
  ...
  } else tibble::tibble(key = character(), Infection_Status = character())
  ...
  episode <- uti |>
    dplyr::transmute(
      ...
      in_vf_analysis_ready = .data$wgs_key %in% unique(vf_ready$key[vf_ready$Infection_Status == "UTI"]) |
        .data$episode_key %in% unique(vf_ready$episode_key[vf_ready$Infection_Status == "UTI"]),
