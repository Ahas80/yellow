...
OUT = Path(
    ...
    "presentations/ruti-longitudinal-vf-pipeline-review-v4/output/"
    "Longitudinal_Urinary_Ecoli_VF_Pipeline_Detailed_Presenter_Script_With_Onboarding_2026-05-28.docx"
)
...
slides[1:1] = [
    {
        ...
        "primary": "Give a newcomer the whole behind-the-scenes loop before technical results appear.",
        "visual": "Move left to right across the six boxes, then use the three lower boxes to separate measurement, comparison, and clinical overlay.",
        "script": [
    ...
    },
    {
        "n": 3,
        "time": "1:00",
        "title": "What changes as data move through the pipeline",
        "primary": "Explain that each denominator refers to a different unit of analysis.",
        "visual": "Use the ladder from clinical episodes to VF-ready episodes, feature space, longitudinal pairs, and clinical transitions.",
        "script": [
    ...
    },
    {
        ...
        "primary": "Define the clinical annotation simply: culture support plus catheter-aware compatible symptoms.",
        "visual": "Show the two criteria feeding into UTI, then the denominator boxes at the bottom.",
        "script": [
...
def add_title_page(doc):
    ...
    add_label_detail_table(
        doc,
        [
            ("Purpose", "Support a scientific review plus discussion for a colleague who needs biological interpretation and project-navigation context.", LIGHT_BLUE, BLUE),
            ...
            ("Core timing", "Slides 1-16 are the spoken narrative. Slides 17-25 are appendix/Q&A backup.", LIGHT_ORANGE, ORANGE),
        ],
    )
    add_callout(
        doc,
...
def build():
    ...
    doc.add_heading("Slides 1-16: Spoken Narrative", level=1)
    for slide in slides:
    ...
    section.left_margin = Inches(0.85)
    section.right_margin = Inches(0.85)
    doc.add_heading("Slides 17-25: Appendix / Q&A Prompts", level=1)
    for slide in appendix:
