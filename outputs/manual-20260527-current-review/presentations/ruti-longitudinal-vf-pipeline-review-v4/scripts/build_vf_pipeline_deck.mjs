...
const OUTPUT = path.join(WORKSPACE, "output", "Longitudinal_Urinary_Ecoli_VF_Pipeline_Review_With_Onboarding_BASE_SHIFTED_EDITABLE_GENERATED_REPAIR_WARNING.pptx");
const PREVIEW_DIR = path.join(WORKSPACE, "preview", "base_shifted");
const LAYOUT_DIR = path.join(WORKSPACE, "layout", "base_shifted");
const CONTACT_SHEET = path.join(WORKSPACE, "preview", "Longitudinal_Urinary_Ecoli_VF_Pipeline_Review_base_shifted_contact_sheet.png");
...
async function build() {
  ...
  for (let i = 0; i < slides.length; i += 1) {
    ...
    clearSlide(slide);
    const displayedSlideNumber = i === 0 ? 1 : i + 4;
    const ctx = createSlideContext({ FileBlob, PresentationFile }, { slideSize: { width: 1280, height: 720 }, slideNumber: displayedSlideNumber, workspaceDir: WORKSPACE, assetDir: path.join(WORKSPACE, "assets"), outputDir: path.dirname(OUTPUT) });
