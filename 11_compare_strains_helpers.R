...
load_core_tables <- function() {
  ...
  # VF presence/absence
  if (!file.exists("results/vf_pa_all.csv"))
    stop("results/vf_pa_all.csv not found – run 02_gene_presence_analysis.R")
  vf_pa <- readr::read_csv("results/vf_pa_all.csv", show_col_types = FALSE)
...
resolve_sample <- function(Participant_id, tp_lab, assemblies, prefer_assembler = "flye") {
  ...
  df <- assemblies %>% filter(Participant_id == !!Participant_id,
...
parse_dnadiff_report <- function(report_path) {
  if (!file.exists(report_path)) return(tibble(AvgIdentity = NA_real_, TotalSNPs = NA_real_))
...
run_dnadiff <- function(a_fasta, b_fasta, cache_dir, key) {
  ...
  if (!has_tool("dnadiff")) return(tibble(AvgIdentity = NA_real_, TotalSNPs = NA_real_))
  ...
  if (!file.exists(rpt)) {
    ...
    status <- system(cmd)
    if (status != 0) return(tibble(AvgIdentity = NA_real_, TotalSNPs = NA_real_))
  }
  parse_dnadiff_report(rpt)
}

mash_distance <- function(a_fasta, b_fasta) {
  if (!has_tool("mash")) return(NA_real_)
...
booleanize_df <- function(df) {
  df[] <- lapply(df, function(col) {
    ...
    if (is.numeric(col)) return(as.integer(col > 0))
...
jaccard_from_wide <- function(df_wide, row_key_cols = c("Participant_id", "tp_lab"), sampleA, sampleB, cols_regex = NULL) {
  ...
  if (!nrow(A) || !nrow(B)) return(list(jaccard = NA_real_, n_int = NA_integer_, n_union = NA_integer_))
  mat <- rbind(A, B) %>% select(-all_of(c(row_key_cols, "SampleKey"))) %>% booleanize_df() %>% as.matrix()
  if (!ncol(mat)) return(list(jaccard = NA_real_, n_int = 0L, n_union = 0L))
  a <- as.integer(mat[1, ] > 0); b <- as.integer(mat[2, ] > 0)
  inter <- sum(a & b); un <- sum(a | b)
...
classify_pair <- function(metrics, thresholds = list(id = 99.9, snps = 50, vf = 0.9, inc = 0.8, vf_rel = 0.7, inc_rel = 0.7)) {
  ...
  if (same_rule) return(list(Classification = "Same", RuleUsed = "ST + ID/SNP + accessory"))
  ...
  if (related_rule) return(list(Classification = "Related", RuleUsed = "ST + partial concordance"))

  # Fallbacks when identity unavailable
  if (!has_id && ST_equal && !is.na(mean_acc)) {
    if (mean_acc >= 0.9) return(list(Classification = "Same", RuleUsed = "ST + accessory >=0.9 (no ID)"))
    if (mean_acc >= 0.7) return(list(Classification = "Related", RuleUsed = "ST + accessory >=0.7 (no ID)"))
  }
