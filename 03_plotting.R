...
load_robust_metadata <- function() {
  ...
  # Dynamically load all available batch CSVs defined in 00_config.R
  batch_parts <- lapply(BATCH_CLINICAL_CSVS[!is.na(BATCH_CLINICAL_CSVS)], function(csv_name) {
    read_batch_safe(file.path("data", "inputs", csv_name))
  })
  batch_parts <- batch_parts[!vapply(batch_parts, is.null, logical(1))]

  bind_rows(batch_parts) %>%
...
# Script 02 now carries provenance/key columns such as Episode_ID in vf_pa_all.
# Treat only numeric binary VF columns as genes; do not pivot metadata columns.
vf_meta_cols <- c(
...
gene_stat_cols <- if ("GENE" %in% names(tbl_gene)) as.character(tbl_gene$GENE) else character()
vf_gene_cols <- canonical_vf_gene_cols(names(vf_pa_all), vf_pa_file = FILE_VF_PA)
if (length(gene_stat_cols) > 0) {
  vf_gene_cols <- intersect(vf_gene_cols, gene_stat_cols)
}
if (length(vf_gene_cols) == 0) {
  stop("No numeric VF gene columns found in ", FILE_VF_PA)
}
vf_pa_all <- vf_pa_all %>%
  mutate(across(all_of(vf_gene_cols), ~ tidyr::replace_na(as.numeric(.x), 0)))
...
if (has_pkg("ComplexUpset")) {
  ...
  if (file.exists(FILE_STATUS_MAP)) {
    ...
    gene_status_df <- vf_pa_all %>%
      pivot_longer(cols = all_of(vf_gene_cols), names_to = "GENE", values_to = "present") %>%
      filter(present > 0) %>%
      select(-present) %>%
...
if (exists("vf_hits_all") && file.exists(FILE_STATUS_MAP)) {
  ...
  message("Canonical VF burden visualisation is generated by 23_vf_cross_sectional.R at plots/vf/vf_burden_by_status.png.")
  writeLines(
    c(
      "Legacy Wilcoxon/stat output skipped by 03_plotting.R.",
      "Use canonical VF outputs instead:",
  ...
  tryCatch(
    {
      ...
      top_vfs <- intersect(top25$GENE[1:min(20, nrow(top25))], vf_gene_cols)
