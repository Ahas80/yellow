...
allele_specific_fisher <- function(gene_name, allele_name, allele_n) {
  ...
  tibble(
    ...
    allele_positive = allele_positive,
    allele_negative = allele_negative,
...
if (nrow(alleles_to_test) > 0) {
  ...
  allele_specific_vs_nitrite_fisher_bh <- tibble(
    gene = character(),
    allele = character(),
    allele_total = integer(),
    allele_positive = integer(),
    allele_negative = integer(),
    other_positive = integer(),
    other_negative = integer(),
    p_value = numeric(),
    bh_q_value = numeric(),
...
summary_lines <- c(
  ...
  "The allele-specific output tests each allele against all other alleles/absent for the same gene and should be treated as secondary exploratory follow-up.",
